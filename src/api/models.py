...
class User(db.Model):
    ...
    password_hash: Mapped[str] = mapped_column(String(255), nullable=False)  # Ampliado para hashes seguros
    is_active: Mapped[bool] = mapped_column(Boolean(), nullable=False, default=True)

    # Relaciones
    articulos: Mapped[list["Articulos"]] = relationship("Articulos", back_populates="usuario", cascade="all, delete-orphan")
    articulos_favoritos: Mapped[list["Articulo_favorito"]] = relationship("Articulo_favorito", back_populates="usuario", cascade="all, delete-orphan")
    ratings: Mapped[list["Rating"]] = relationship("Rating", back_populates="usuario", cascade="all, delete-orphan")
    datos_personales: Mapped[list["DatosPersonales"]] = relationship("DatosPersonales", back_populates="usuario", cascade="all, delete-orphan")
    ...
    transacciones_como_user1: Mapped[list["TransaccionTrueke"]] = relationship("TransaccionTrueke", foreign_keys="TransaccionTrueke.user1_id", back_populates="user1", cascade="all, delete-orphan")
    transacciones_como_user2: Mapped[list["TransaccionTrueke"]] = relationship("TransaccionTrueke", foreign_keys="TransaccionTrueke.user2_id", back_populates="user2", cascade="all, delete-orphan")
...
<<<<<<< HEAD
    ...
    categoria: Mapped[str] = mapped_column(Enum('electronica', 'ropa', 'hogar', 'deportes', 'libros', 'juguetes', 'otros', name='categoria_enum'), nullable=False)
...
>>>>>>> cad1603986c52be1a9d91994b016b72aaccdb70e
    ...
    transaccion_trueke: Mapped["TransaccionTrueke"] = relationship("TransaccionTrueke", back_populates="articulo", uselist=False)
...
class Articulo_favorito(db.Model):
    ...
    # Relaciones
    usuario: Mapped['User'] = relationship('User', back_populates='articulos_favoritos')
    articulo: Mapped['Articulos'] = relationship('Articulos', back_populates='articulos_favoritos')

    def __str__(self):
        return f"Favorito: {self.articulo.titulo} by {self.usuario.nombre_de_usuario}"
...
class Comentarios(db.Model):
    ...
    # Foreign Keys
    comentario_user_id: Mapped[int] = mapped_column(ForeignKey('usuario.id'), nullable=False)
    transaccion_id: Mapped[int] = mapped_column(ForeignKey('transaccion_trueke.id'), nullable=False)
    
    # Relaciones
    usuario: Mapped['User'] = relationship('User', back_populates='comentarios')
    transaccion: Mapped['TransaccionTrueke'] = relationship('TransaccionTrueke', back_populates='comentarios_transaccion')
...
class DatosPersonales(db.Model):
    ...
    # Relaciones
    usuario: Mapped['User'] = relationship('User', back_populates='datos_personales')
...
class Rating(db.Model):
    ...
    articulo: Mapped['Articulos'] = relationship('Articulos', back_populates='ratings')
